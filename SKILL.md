...
```
Experimental: 1:1 sizing, editing and one-page-per-card PDF work; there is no per-platform export (PNG per card) yet — `node bin/pdf.mjs deck.html` gives one card per page (vector), or screenshot from `verify-out/`.
