// decklet template library gate — the 59 candidates ship in the engine as `template:` slides. A template is a finished
...
test('templates: 59 ship, every id unique, every one names a tier, a category and a density', () => {
  assert.equal(TEMPLATES.length, 59);
  assert.equal(new Set(TEMPLATES.map(t => t.id)).size, 59);
...
test('templates: keys — every text row gets t1..tn in row order; the catalogue prints each with its sample', () => {
  const k = templateKeys('logic-tree');
  assert.ok(k.length >= 5, 'the tree has a question and branches');
  ...
  assert.match(cat, /logic-tree/); assert.match(cat, /t1/); assert.match(cat, /Why did margin fall\?/);
...
test('templates: expand — a template slide becomes the template rows, fill overrides by key, free rows are kept after', () => {
  const d = deck([{template: 'logic-tree', fill: {t2: 'Why did churn rise?'}, els: [{x: 60, y: 480, w: 800, role: 'Caption', text: 'free row'}]}]);
  expandTemplates(d);
  const s = d.slides[0];
  ...
  assert.ok(texts.includes('Why did churn rise?'), 'fill replaced t2');
  assert.ok(!texts.includes('Why did margin fall?'), 'the sample is gone');
...
test('templates: an unknown template is an error that lists the library; a fill key that does not exist is an error', () => {
  ...
  assert.ok(r.errors.some(m => /template "no-such"/.test(m) && /logic-tree/.test(m)), r.errors.join(' | '));
