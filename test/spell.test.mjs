...
test('spell: the toggle wears a count badge, every contact-sheet cell carries one, and the HUD manifest names it', () => {
  assert.match(tpl, /<button id="spell"[\s\S]*?<span id="spellbad" role="status" hidden><\/span><\/button>/, 'the badge lives inside the spellcheck button, as the autosave dot lives inside versions');
