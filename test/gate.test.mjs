...
test('⤓ PDF writer: slide-sized pages from foreignObject rasters, byte-exact xref, real link annotations, zero dependencies', () => {
  for (const re of [/async function exportPdf\(\)\{\n\s*commitEdit\(\)/, /<foreignObject width="\$\{W\}" height="\$\{H\}">/, /'data:image\/svg\+xml;charset=utf-8,'\+encodeURIComponent\(svg\)/, /c\.toDataURL\('image\/jpeg',\.92\)/, /new CompressionStream\('deflate'\)/, /\/Filter \$\{j\.f\} \/Length \$\{j\.d\.length\}/, /S=3,pics=\[\]/, /\/MediaBox \[0 0 \$\{W\} \$\{H\}\]/, /\/Filter \$\{j\.f\}/, /startxref\\n\$\{x\}\\n%%EOF/, /type:'application\/pdf'/, /\.pdf';a\.click\(\)/]) assert.match(tpl, re, String(re));
