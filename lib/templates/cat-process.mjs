...
// The block-arrow flow at three lengths. ONE shape: n boxes and n-1 arrows across the 816px span the four-box cut has always
// used, with a 32px gap for each arrow — so the width falls out of the count (3 → 251, 4 → 180, 5 → 138) and the four-box
// numbers are exactly the ones that shipped. Keep a caption under about 34 characters: at five boxes it is a 106px column.
const SPAN = 816, ARROW = 32;
const flow = (steps, tail) => { const n = steps.length, w = Math.round((SPAN - ARROW * (n - 1)) / n), pitch = w + ARROW;
  return [ ...steps.flatMap(([h, s], i) => { const x = M + i * pitch; const r = [
      { id: `st${i}`, ...rect(x, 200, w, 132, { bg: 'var(--card)', bd: '1px solid var(--line)', radius: 8 }) },
      lab(x + 16, 216, 60, `0${i + 1}`, { color: 'var(--accent)' }),
      t(x + 16, 240, w - 32, 'H2', h), cap(x + 16, 274, w - 32, s) ];
      if (i < n - 1) r.push({ x: x + w, y: 266, line: [x + pitch, 266], h: 2.5, bg: 'var(--fg)', arrow: 'end', to: `st${i + 1}`, gap: 6 });
      return r; }),
    rule(M, 386, W - M), lab(M, 400, 500, tail) ]; };

export default [
{ id: 'process-flow-3', name: 'Process flow — three boxes', tier: 'core', cat: 'Process', note: 'Three linear steps with real arrow heads stopped on the border.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'The close' }, { slot: 'title', text: 'Three steps, one morning.' },
    ...flow([['Import', 'The bank feed lands overnight'], ['Match', 'Payments meet invoices by nine'], ['Post', 'The ledger closes before lunch']],
      'A person touches only the exceptions') ] },
...
{ id: 'process-flow-4', name: 'Process flow — four boxes', tier: 'core', cat: 'Process', note: 'Linear steps with real arrow heads stopped on the border.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'How it runs' }, { slot: 'title', text: 'Four steps, one business day.' },
    ...flow([['Order', 'Chef builds the basket'], ['Confirm', 'Farms confirm by 6 pm'], ['Pick', 'Packed at the hub overnight'], ['Deliver', 'On the dock by 7 am']],
      'A person touches step three only') ] },

{ id: 'process-flow-5', name: 'Process flow — five boxes', tier: 'core', cat: 'Process', note: 'Five linear steps with real arrow heads stopped on the border.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'Delivery' }, { slot: 'title', text: 'Five hops, under a second.' },
    ...flow([['Accept', 'Event in, 8 ms'], ['Queue', 'Held per endpoint'], ['Sign', 'A hash on the body'], ['Send', 'One try, five seconds'], ['Retry', 'Backs off over an hour']],
      'A failure waits in the queue, never silent') ] },
...
{ id: 'timeline-horizontal', name: 'Timeline — milestones on a spine', tier: 'core', cat: 'Process', note: 'One spine, dated dots, labels alternating above and below.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'Plan' }, { slot: 'title', text: 'Pilot to production in five months.' },
    ...
    ...[['Sep', 'Pilot signed'], ['Oct', 'Units installed'], ['Nov', 'First alerts'], ['Dec', 'Insurer report'], ['Jan', 'Production']]
...
{ id: 'gantt-lanes', name: 'Gantt — swimlanes', tier: 'core', cat: 'Process', note: 'Three lanes, bars on a month grid, one hairline for today.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'Schedule' }, { slot: 'title', text: 'Two tracks overlap in November.' },
    ...
    ...[['Queue rewrite', 0, 3, 'var(--accent)'], ['Dashboard', 2, 2, 'var(--muted)'], ['Migration', 3, 2, 'var(--accent)']]
...
{ id: 'vertical-steps', name: 'Vertical steps — numbered rail', tier: 'core', cat: 'Process', note: 'A rail with numbered stops; reads top to bottom like a checklist.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'Next' }, { slot: 'title', text: 'What happens after you sign.' },
    ...
    ...[['Kickoff', 'Two people, ninety minutes.'], ['Discover', 'Two weeks inside your team.'], ['Design', 'One operating cadence, on paper.'], ['Go live', 'The first sprint runs the same month.']]
...
{ id: 'cycle-loop', name: 'Cycle — four-stage loop', tier: 'standard', cat: 'Process', note: 'Four nodes, four curved arrows, no beginning. For anything iterative.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'Training loop' }, { slot: 'title', text: 'The loop a runner never leaves.' },
    { id: 'n0', ...box(395, 160, 170, 62, 'H2', 'Run') },
    { id: 'n1', ...box(680, 272, 170, 62, 'H2', 'Track') },
    { id: 'n2', ...box(395, 384, 170, 62, 'H2', 'Coach') },
    { id: 'n3', ...box(110, 272, 170, 62, 'H2', 'Adjust') },
    ...
    body(M, 470, 400, 'Every run tunes the next week\'s plan.', { color: 'var(--muted)' }) ] },
...
{ id: 'funnel-stages', name: 'Funnel — narrowing stages', tier: 'core', cat: 'Process', note: 'Centred bands that shrink, with the drop-off named on the right.', layout: 'content',
  els: [ { slot: 'supertitle', text: 'Conversion' }, { slot: 'title', text: 'The leak is between sign-up and first run.' },
    ...[['Downloads', 620, '12,400'], ['Signed up', 500, '9,900'], ['First run', 300, '5,100'], ['Paid', 180, '1,600']]
    ...
    lab(768, 300, 132, '−48% here', { color: 'var(--accent)', nowrap: 1 }) ] },   // 8px off the band
];
