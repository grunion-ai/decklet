...
// A scorecard cell, inside the column box [x, y, w, h]: a 0..4 rating draws the harvey-balls ball (0 · 25 · 50 · 75 · 100
// percent of one 44px donut drawn `hole: 0`, so it reads as a filled ball — at 36px through a hairline ring, three quarters
// and four are the same mark), anything else is short text centred in the column. The box rides along on `cell`, so `fill`
// can swap one form for the other from the key alone (lib/templates.mjs) and the ball lands centred where the text was.
export const RING = 44;
export const cell = (x, y, w, h, v) => typeof v === 'number'
  ? { x: x + (w - RING) / 2, y: y + (h - RING) / 2, w: RING, donut: v * 25, hole: 0, color: 'var(--accent)', cell: [x, y, w, h] }
