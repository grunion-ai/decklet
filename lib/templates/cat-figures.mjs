// Figures — the figure kinds as templates: decision, flow, before / after, data model, states, release, boundaries
// (five nodes, and seven across three zones), tree, layers. Every sample is a spec through lib/diagram.mjs, so the rows are the same rows an agent gets from
...
// The advanced architecture all four study subjects hand-built (ROADMAP U5.6): three zones, seven nodes, six labelled
// edges. Every cross-zone edge is level and every in-zone edge shares a column, so each label sits on its own run and
// none lifts to Y(tops)-22, where it would land on a group label at Y(g.y)+6. The channels are cut for the labels they
// carry: 104px between the zone columns against `checkout` (8 characters = 72 + 16 of air), 100px between the rows.
fig('figure-boundaries-6', 'Boundaries — seven nodes across three zones', 'standard', 'The advanced architecture: three dashed zones, seven nodes, six labelled edges, the money path in accent.', {
  label: 'A card payment crosses three boundaries: the storefront, our platform, the partner bank',
  groups: [{x: 0, y: 8, w: 176, h: 300, label: 'storefront'}, {x: 264, y: 8, w: 292, h: 300, label: 'our platform'},
    {x: 636, y: 8, w: 200, h: 300, label: 'partner bank'}],
  nodes: [node('buyer', 8, 40, 'Buyer', 'one session', '', {w: 144}), node('basket', 8, 240, 'Basket', 'one order', '', {w: 144}),
    node('ledger', 272, 40, 'Ledger', 'posted same day', 'chosen', {w: 260}), node('orders', 272, 140, 'Orders', 'one queue, one owner', '', {w: 260}),
    node('api', 272, 240, 'Checkout', 'takes the payment', 'chosen', {w: 260}),
    node('settle', 644, 40, 'Settlement', 'card network', '', {w: 168}), node('payout', 644, 240, 'Payout account', 'cash in', '', {w: 168})],
  edges: [{from: 'buyer', to: 'basket', label: 'adds items'}, {from: 'basket', to: 'api', state: 'chosen', label: 'checkout'},
    {from: 'api', to: 'orders', label: 'creates'}, {from: 'orders', to: 'ledger', label: 'posts'},
    {from: 'ledger', to: 'settle', state: 'chosen', label: 'settles'}, {from: 'settle', to: 'payout', label: 'cash in'}],
}, 'Boundaries', 'Seven nodes, three boundaries', 'The accent path is the money: a basket becomes a payment, the payment becomes a ledger line, and the ledger is what the bank settles against.'),
