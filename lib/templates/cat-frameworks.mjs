import { M, CW, W, t, lab, cap, body, rect, rule, vrule, dot, tile, box, cols, cell } from './kit.mjs';
...
// Criteria down the side, options across the top, and every one of the twelve cells is a fill key: pass a string for a
// figure or a phrase, a 0..4 rating for the harvey-balls ring. Rows are 62 apart on a 56-high cell, so a ring (36) and a
// hairline keep their air; the `source` slot carries the dense chrome a reading slide owes its reader.
{ id: 'scorecard-grid', name: 'Scorecard — criteria × options', tier: 'core', cat: 'Frameworks', note: 'Criteria down, options across; each cell takes short text or a 0–4 rating drawn as a ring.', layout: 'content',
  els: (() => { const COL = [360, 547, 734], CWD = 166, RH = 56;
    const rowY = i => 216 + i * 62;
    return [ { slot: 'supertitle', text: 'Shortlist' }, { slot: 'title', text: 'Four criteria, three platforms, one page.' },
      lab(M, 176, 280, 'What we scored'),
      ...['Tallyline', 'Relay', 'Fieldsense'].map((h, i) => lab(COL[i], 176, CWD, h, { align: 'center', nowrap: 1 })),
      rule(M, 200, W - M, { bg: 'var(--fg)', h: 2 }),
      ...[['Coverage', [2, 3, 4]], ['Time to live', ['6 weeks', '3 weeks', '2 weeks']],
          ['Price', ['$14 / order', '$11 / order', '$6 / order']], ['Support', [3, 2, 4]]]
        .flatMap(([crit, vals], i) => { const y = rowY(i);
          return [ body(M, y + 18, 280, crit), ...vals.map((v, j) => cell(COL[j], y, CWD, RH, v)), rule(M, y + RH, W - M) ]; }),
      { slot: 'source', text: 'Scored by the buying team · a full ring is 4, an empty one 0' } ]; })() },
