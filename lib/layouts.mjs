...
// THE GRID: every slot edge (x, x+w, y, y+h) is a multiple of GRID (16px), the margin included (64 / 896). The editor's
// guide lines are these edges, and its 16px grid overlay is the same lattice — so a row snapped to a guide is on the grid,
// and a row snapped to the grid can reach a guide. libraryFor() keeps the lattice when it scales to another canvas.
// library: import {LIBRARY, GROUPS, GRID, libraryFor, catalogue} from './layouts.mjs'
export const GROUPS = ['openers', 'text', 'visuals', 'numbers', 'diagrams', 'plans', 'closers'];
export const GRID = 16;
const M = 64, CW = 832;   // margin + content width on the 960 canvas (960 − 2·64), both on the grid
const sup = (x = M, y = 48, w = CW) => ({x, y, w, role: 'Supertitle'});
const h1 = (x = M, y = 80, w = CW) => ({x, y, w, role: 'H1'});
...
const tiles = (n, w, pitch, role = 'Stat2') => {   // Stat2: the KPI allowance — a smaller stat for tiles; the hero `stat` layout keeps Stat
  ...
  for (let k = 0; k < n; k++) {
    const x = M + k * pitch;
    o[`kpi${k + 1}`] = {x, y: 160, w, h: 128, tile: 1, role, align: 'center'};
    o[`kpi${k + 1}-delta`] = {x: x + 16, y: 176, w: 'auto', role: 'Label', p: 'chip', radius: 4, nowrap: 1, bg: 'var(--box)', color: 'var(--ok,var(--accent))'};
    o[`kpi${k + 1}-label`] = {x, y: 304, w, role: 'Label', align: 'center'};
  }
  return o;
};
const cols = (n, mk) => Object.fromEntries([...Array(n)].flatMap((_, k) => mk(k + 1, M + k * 208)));   // k, x at a 208 pitch (192 wide + 16 gap)

export const LIBRARY = {
  // ── openers
  cover:   {group: 'openers', density: 'speaker', use: 'Open the deck: name, one-line promise, who and when.',
    slots: {supertitle: sup(M, 160), title: {x: M, y: 192, w: CW, role: 'Title'}, body: {x: M, y: 384, w: 704, role: 'Body', color: 'var(--muted)'}, caption: {x: M, y: 448, w: CW, role: 'Caption'}}},
  agenda:  {group: 'openers', density: 'reading', use: 'List what the deck covers, up to five numbered items.',
    slots: {...head, ...cols(5, (k) => [[`n${k}`, {x: M, y: 160 + (k - 1) * 64, w: 48, role: 'Label'}], [`item${k}`, {x: M + 48, y: 160 + (k - 1) * 64, w: 784, role: 'Body'}]])}},
  section: {group: 'openers', density: 'speaker', use: 'Divide the deck: a section number and its title, with one line of context.',
    slots: {number: {x: M, y: 192, w: CW, role: 'Label'}, title: {x: M, y: 224, w: CW, role: 'Title'}, body: {x: M, y: 400, w: 704, role: 'Body', color: 'var(--muted)'}}},
  // ── text
  statement: {group: 'text', density: 'speaker', use: 'Make one claim in display type, alone on the slide.',
    slots: {title: {x: M, y: 176, w: CW, role: 'Title'}, caption: {x: M, y: 416, w: CW, role: 'Caption'}}},
  fact:    {group: 'text', density: 'speaker', use: 'Lead with a number, then name what it is and why it matters.',
    slots: {stat: {x: M, y: 144, w: CW, role: 'Stat'}, label: {x: M, y: 256, w: CW, role: 'Label'}, body: {x: M, y: 288, w: 704, role: 'Body'}}},
  quote:   {group: 'text', density: 'speaker', use: 'Quote someone: the words in italic, the attribution beneath.',
    slots: {quote: {x: M, y: 160, w: CW, role: 'H2', italic: 1}, attribution: {x: M, y: 336, w: CW, role: 'Caption'}}},
  'two-cols': {group: 'text', density: 'reading', use: 'Set two bodies of text side by side under one title.',
    slots: {...head, left: {x: M, y: 160, w: 400, role: 'Body'}, right: {x: 496, y: 160, w: 400, role: 'Body'}}},
  'two-cols-header': {group: 'text', density: 'reading', use: 'Set a spanning lede above two columns of text.',
    slots: {...head, header: {x: M, y: 144, w: CW, role: 'H2'}, left: {x: M, y: 208, w: 400, role: 'Body'}, right: {x: 496, y: 208, w: 400, role: 'Body'}}},
  // ── visuals
  'image-left': {group: 'visuals', density: 'reading', use: 'Pair an image on the left with a title and text on the right.',
    slots: {image: {x: M, y: 144, w: 400, h: 320}, supertitle: sup(496, 144, 400), title: h1(496, 176, 400), body: {x: 496, y: 256, w: 400, role: 'Body'}}},
  'image-right': {group: 'visuals', density: 'reading', use: 'Pair a title and text on the left with an image on the right.',
    slots: {supertitle: sup(M, 144, 400), title: h1(M, 176, 400), body: {x: M, y: 256, w: 400, role: 'Body'}, image: {x: 496, y: 144, w: 400, h: 320}}},
  // ── numbers
  'kpi-grid': {group: 'numbers', density: 'reading', use: 'Show three KPIs as tiles, each a value with a label and a delta chip.',
    slots: {...head, ...tiles(3, 256, 288), body: {x: M, y: 352, w: CW, role: 'Body'}}},
  'kpi-grid-4': {group: 'numbers', density: 'reading', use: 'Show four KPIs as tiles, each a value with a label and a delta chip.',
    slots: {...head, ...tiles(4, 192, 208), body: {x: M, y: 352, w: CW, role: 'Body'}}},
  stat:    {group: 'numbers', density: 'speaker', use: 'State one hero number at the deck Stat size, with a title above and a caption beneath.',
    slots: {...head, stat: {x: M, y: 176, w: CW, role: 'Stat', align: 'center'}, caption: {x: M, y: 304, w: CW, role: 'Caption', align: 'center'}}},
  chart:   {group: 'numbers', density: 'reading', use: 'Plot one series (a chart row in the chart slot), then say the takeaway and the source.',
    slots: {...head, chart: {x: M, y: 128, w: CW, h: 288}, takeaway: {x: M, y: 432, w: CW, role: 'Body'}, source: {x: M, y: 464, w: CW, role: 'Caption'}}},
  comparison: {group: 'numbers', density: 'reading', use: 'Compare two options: a heading and a body for each, side by side.',
    slots: {...head, 'left-head': {x: M, y: 144, w: 400, role: 'H2'}, 'right-head': {x: 496, y: 144, w: 400, role: 'H2'}, left: {x: M, y: 192, w: 400, role: 'Body'}, right: {x: 496, y: 192, w: 400, role: 'Body'}}},
  // ── diagrams
  'process-steps': {group: 'diagrams', density: 'reading', use: 'Walk through up to four numbered steps as a row of tiles.',
    slots: {...head, ...cols(4, (k, x) => [[`n${k}`, {x, y: 160, w: 192, role: 'Label'}], [`step${k}`, {x, y: 192, w: 192, h: 128, tile: 1, role: 'Body'}]]), body: {x: M, y: 352, w: CW, role: 'Body'}}},
  // ── plans
  timeline: {group: 'plans', density: 'reading', use: 'Place up to four dated events along one rule.',
    slots: {...head, rule: {x: M, y: 240, w: CW, h: 2, line: [896, 240], bg: 'var(--line)'},
      ...cols(4, (k, x) => [[`d${k}`, {x, y: 224, w: 16, h: 16, radius: 8, bg: 'var(--accent)'}], [`t${k}`, {x, y: 256, w: 192, role: 'Label'}], [`e${k}`, {x, y: 272, w: 192, role: 'Body'}]])}},
  // ── closers
  cta:     {group: 'closers', density: 'speaker', use: 'Ask for the one next step, with a painted button that carries the link.',
    slots: {title: {x: M, y: 144, w: CW, role: 'Title'}, body: {x: M, y: 320, w: 704, role: 'Body'}, button: {x: M, y: 400, w: 256, h: 48, bg: 'var(--accent)', radius: 8}, 'button-label': {x: M, y: 416, w: 256, role: 'Body', align: 'center', weight: 600, color: 'var(--card)'}}},
  end:     {group: 'closers', density: 'speaker', use: 'Close the deck: thanks, how to reach you, and a last caption.',
    slots: {title: {x: M, y: 192, w: CW, role: 'Title'}, body: {x: M, y: 304, w: 704, role: 'Body'}, caption: {x: M, y: 448, w: CW, role: 'Caption'}}},
};

// the layouts a deck references but does not define, scaled from the 960×540 cut to the deck's canvas — and re-seated on
// the 16px lattice after scaling, so a 1600×900 deck's guides are grid lines too (1.67× alone would leave them between)
export function libraryFor(deck) {
  ...
  const scale = sl => Object.fromEntries(Object.entries(sl).map(([k, v]) => [k,
    (k === 'x' || k === 'w') && typeof v === 'number' ? q(v * sx) : (k === 'y' || k === 'h') && typeof v === 'number' ? q(v * sy)
    : k === 'line' ? [q(v[0] * sx), q(v[1] * sy)] : v]));
