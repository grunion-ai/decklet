...
export const DENSE = {
  ...
  source:   {x: M, y: 474, w: 500, role: 'Caption'},                                   // where the numbers came from, left foot (ends 492, above the footer; 500 wide leaves the legend its room)
  legend:   {right: LEGEND_R, y: 474, w: 'auto', role: 'Label', nowrap: 1},            // series or symbol key, right foot, left of the counter
};
...
export const LIBRARY = {
  ...
  bullets: {group: 'text', density: 'reading', use: 'Make four to six points under one title, one line each, a dot per point.',
    slots: {...head, ...bullets(6, M + 36, 160, 780, 44)}},   // bind three or fewer and the slide is a speaker slide; six is the reading ceiling
  // ── visuals
  ...
  'image-left': {group: 'visuals', density: 'reading', use: 'Pair an image on the left with a title and a paragraph or points on the right.',
    ...
    dense: {subtitle: {x: 500, y: 250, w: 400, role: 'H2', color: 'var(--muted)', nowrap: 1}, note: {x: 500, y: 424, w: 400, role: 'Body', color: 'var(--muted)'}, source: {x: M, y: 476, w: 500, role: 'Caption'}, legend: {right: LEGEND_R, y: 476, w: 'auto', role: 'Label', nowrap: 1}}},
  'image-right': {group: 'visuals', density: 'reading', use: 'Pair a title and a paragraph or points on the left with an image on the right.',
    ...
    dense: {subtitle: {x: M, y: 250, w: 400, role: 'H2', color: 'var(--muted)', nowrap: 1}, note: {x: M, y: 424, w: 400, role: 'Body', color: 'var(--muted)'}, source: {x: M, y: 476, w: 500, role: 'Caption'}, legend: {right: LEGEND_R, y: 476, w: 'auto', role: 'Label', nowrap: 1}}},
  ...
  chart:   {group: 'numbers', density: 'reading', use: 'Plot one series (a chart row in the chart slot), then say the takeaway and the source.',
    slots: {...head, chart: {x: M, y: 150, w: CW, h: 262}, takeaway: {x: M, y: 420, w: CW, role: 'Body'}, source: {x: M, y: 476, w: 500, role: 'Caption'}},
    dense: {note: false, legend: {right: LEGEND_R, y: 476, w: 'auto', role: 'Label', nowrap: 1}}},   // takeaway is the note (two lines fit above the source); the media moved down 14px to seat the subtitle
...
const wordsOf = r => plainOf(r).split(/\s+/).filter(Boolean).length;
...
export function densityReport(s, layouts, density) {
  ...
  const overWords = k => say(`${k} density carries ≤ ${d.max.words} words, this slide has ${words}`,
  ...
  if (words > d.max.words) return overWords('reading');
  return null;
}
