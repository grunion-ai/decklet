...
export function chartRows(r, roles) {
  ...
  // a column's label box is the PITCH between this tick and the next, never the slot rounded up: at 718/4 = 179.5 the
  // rounded 180 overran the neighbour by a pixel and the gap gate called it a collision on every kit in library.html
  const cw = k => (k + 1 < n ? sx(k + 1) : Math.round(x0 + pw)) - sx(k);
  ...
  if (c.mark === 'bar') {
    ...
    data.forEach((d, k) => {
      ...
      const put = (bx, v, series) => { const by = py(v);
        ...
        rows.push(label({x: cmp ? bx : sx(k), y: by - L - 4, w: cmp ? bw : cw(k), align: 'center', color: 'var(--fg)'}, series ? fmt(v) : (d.text ?? fmt(v)))); };
      put(bx, d.value, 0); if (cmp && isNum(d.compare)) put(bx + bw + 4, d.compare, 1);
      rows.push(label({x: sx(k), y: base + 8, w: cw(k), align: 'center'}, d.label));
    });
  } else {
    ...
    data.forEach((d, k) => rows.push(label({x: sx(k), y: base + 8, w: cw(k), align: 'center'}, d.label)));
  }
  if (c.source) rows.push({x, y: y + h - C, w, role: 'Caption', text: c.source});
  return r.group == null ? rows : rows.map(o => ({...o, group: r.group}));   // the chart moves as one in the editor
}
...
export function expandCharts(deck) {
  ...
  for (const s of deck.slides || []) {
    ...
    for (let k = s.els.length - 1; k >= 0; k--) {
      ...
      const g = {x: r.x ?? sl.x ?? 0, y: r.y ?? sl.y ?? 0, w: r.w ?? sl.w ?? 0, h: r.h ?? sl.h ?? 0, group: r.group ?? r.id ?? `chart${k + 1}`};
