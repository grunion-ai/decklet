// decklet template library — 59 finished slides that ship in the engine. A template is a layout PLUS sample rows: the
// candidate sheet (templates/candidates.html) surveyed across the open-source slide catalogs, reviewed on
// 2026-09-07 and promoted whole. A slide names one — `{template:'logic-tree', fill:{t2:'Why did churn rise?'}}` — and
...
const SPEAKER = new Set(['cover-hero', 'cover-split', 'section-numeral', 'quote-pull', 'statement', 'closing-cta', 'stat-hero', 'delta-pair',
  'chart-donut', 'chart-gauge', 'chart-flow-split', 'image-hero-overlay', 'image-split', 'cycle-loop', 'logic-tree']);
