...
// of the value map. `fill` takes those keys as numbers, validate range-checks them, and `--templates` prints them under
// the text keys.
...
const driven = t => { if (!t.build) return new Set();
  const far = Object.fromEntries(Object.entries(t.vals).map(([k, d]) => [k, d.sample === d.range[1] ? d.range[0] : d.range[1]]));
...
export function templateCatalogue() {
  ...
  for (const cat of [...new Set(TEMPLATES.map(t => t.cat))]) {
    ...
    for (const t of TEMPLATES.filter(t => t.cat === cat)) {
      ...
      for (const v of templateVals(t.id)) lines.push(`    ${v.key.padEnd(4)} ${`${v.range[0]}–${v.range[1]}`.padEnd(10)} ${v.of}`);
    }
  }
  return lines.join('\n');
}
