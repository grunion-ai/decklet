// decklet template library — 68 finished slides that ship in the engine (59 candidates + the nine figures). A template is a layout PLUS sample rows: the
...
export function templateFixed(id) {
  const t = TEMPLATE[id]; if (!t) return null;
  const keyed = new Set(t.els.filter(isText));   // templateKeys() draws one key per text row, in the same order
  const n = {};
