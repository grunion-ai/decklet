...
// the marked JSON blocks a deck file carries: DECK (the model), LOG (human edits). Anchored to the data block
// (`const X=/*X*/…/*/X*/`): fileHtml()'s own source holds the same marker strings as quoted literals, and a file from
// before 0.5.0 has no LOG data block at all — unanchored, that literal was the match (ROADMAP P1.4). A file from 0.5.0–0.8.x
// also carries a VERSIONS block (the in-file history, retired in 0.9.0 — that lives in weave now); create --from ignores it.
